# Rune — the `auth` crate (authn + authz) — DRAFT SPEC

Status: **draft / not implemented.** `auth` is a **standalone crate** — authentication (users,
sessions, login, password hashing) *and* authorization (a small gate trait + presets) in one place.
It's useful **on its own** (gate any axum app), and `autocrud` *optionally* depends on it to gate the
generated API + admin. It also keeps the door open for 2FA (TOTP / PassKeys), OIDC SSO, and
app-defined API tokens.

**Dependency direction:** `autocrud → auth` (autocrud pulls `auth` behind a feature to gate its
handlers). `auth` never depends on `autocrud`, so you can use `auth` without any of autocrud.

Sibling docs: [docs/AUTOCRUD.md](AUTOCRUD.md) (the API/UI), [PRD.md](../PRD.md) (roadmap).

## 1. Goals & principles

- **Standalone.** `auth` gates any axum app on its own; autocrud is just one consumer. (authn and
  authz live together — authz is only a trait + a few impls, not worth its own crate/namespace.)
- **One identity, everywhere.** The same authenticated principal gates the autocrud API, the admin
  UI, *and* the app's own handlers — via one extractor + one authorization trait.
- **The app owns the roots.** As with the router / shell / OpenAPI (see AUTOCRUD.md § Composing with
  your app), auth is applied *by the app* to its router. `auth` provides middleware layers,
  extractors, login routes, the gate trait, and SeaORM models — the app wires them where it wants, so
  it can leave `/metrics` public, IP-gate an internal API, or bearer-auth its own namespace.
...
- **Don't shut doors.** The principal is resolved from *pluggable* credential sources; the session
  cookie is the built-in, and Bearer/API-token / OIDC sources slot in later without changing the
  gate or the app's call sites.
...
client → [real-ip] → [request logging] → [CORS] → [session] → [authn: resolve Principal]
       → [CSRF for cookie-auth writes] → router
                                          ├─ autocrud routes   (gated by Authz)
...
- **authz** is consulted per (operation, model) inside the autocrud handlers, and is callable from
  the app's own handlers.

Everything lives in the `auth` crate: the `Principal` / `Authz` contract (dependency-light) plus the
SeaORM users/sessions + login + hashing + middleware (behind an `auth` feature that pulls SeaORM,
argon2, cookies). `autocrud` reads `auth::Principal` from the request extensions and calls the
`auth::Authz` gate — its dependency on `auth` is what feature `auth` turns on (see §9).

## 3. Identity mechanism — DECIDED: server-side session

**Server-side session, carried in an opaque cookie.** A random session id in a
`Set-Cookie: HttpOnly; Secure; SameSite=Strict` cookie, backed by a SeaORM `session` table
(user id, created/expires, and later a 2FA/assurance level + IP/UA).
...
| CSRF | needs SameSite (+ optional token) | same | none (no ambient cookie) |
...
## 4. Middleware the library provides
...
## 5. authn — users, sessions, login, passwords (`autocrud::authn`, feature `authn`)
...
These are ordinary autocrud-registerable entities (so the admin can manage users/groups), with
...
- **Login page:** a server-rendered `username` + `password` form component (askama fragment, like
  `alpine::Table`) posting to a Rune-provided login handler that verifies the hash, creates a
...
- **Password change:** a self-service component (verify current password → set new hash), easy to
  ...
  **admin-group** user it can target *another* user (reset without the current password), so admins
  can change anyone's password. The **admin group name is configurable** (default e.g. `"admin"`).

## 6. authz — the gate (`engine`, ORM-neutral)
...
- **`Open`** — everything allowed (no auth); the default when no gate is set, preserving today's
  behavior.
...
engine; because every method receives the model slug, one impl can still branch per model when it
needs to. There is no separate per-model registration in v1 — a single instance keeps the app-side
API tiny and lets the app share the *same* gate with its own handlers:

```rust
let gate: Arc<dyn Authz> = Arc::new(ReadAllWriteGroups {
    write_groups: vec!["editors".into(), "admin".into()],
});
let crud = Crud::new(db, "/api/v1").authz(gate.clone());   // one gate, all models
// … the app's own handlers authorize with the same `gate.clone()`.
```

(Default when `.authz(..)` is never called: `Open` — today's behavior, unchanged.)

**Enforcement:** each autocrud handler extracts `Option<Principal>` from request extensions and
...
```rust
use auth::{Auth, Authz, CurrentUser, UsersReadGroupWrite};
use std::sync::Arc;
...
// 2. authz: one gate, shared by autocrud and the app's own handlers.
let gate: Arc<dyn Authz> = Arc::new(UsersReadGroupWrite {
    write_groups: vec!["editors".into(), "admin".into()],
});

// 3. autocrud, gated by the shared gate.
...
`auth.routes()` are the login/logout/password endpoints; `CurrentUser` is the extractor backed by
the session the layer resolved. Anything the app wants to leave open (e.g. `/metrics`) is simply not
...
- The `alpine` admin's `fetch` calls add the header automatically (the token is embedded in the
...
- **App API tokens:** the app issues tokens and adds a **principal source** that maps a Bearer token
  → `Principal`; the gate and all call sites are unchanged. Rune ships the session source; token
  sources are app- or future-provided.

## 9. Crate / feature layout

**`auth` is its own crate** (workspace member; published name likely `rune-auth`). It holds both
authn and authz, split by feature so the authorization contract is dependency-light:

- **default / `authz`** — `Principal`, `WriteOp`, `Authz` + presets (`Open`, `ValidUsers`,
  `UsersReadGroupWrite`, …). Minimal deps; this is all a consumer needs to *gate*.
- **`session` (authn)** — SeaORM `user`/`group`/`session` models, argon2id hashing, the session
  ...
  `axum`, `tower-http`, a cookie lib, `rand`, `time` (+ an XFF parser or `axum-client-ip`).

**`autocrud`** gains an optional **`auth` feature** that depends on `auth` (authz part only) so its
handlers can read `auth::Principal` and consult the `auth::Authz` gate. Without the feature, autocrud
is `Open` (unchanged). `auth` never depends on `autocrud`.

```
auth  (authz contract ── + session feature ─→ SeaORM/argon2/axum)
  ▲
  │ feature "auth" (authz only)
autocrud
```

The **app** depends on `auth` (with `session`) for the full stack and on `autocrud` (with `auth`) to
gate the generated API — both see the same `auth::Authz` / `auth::Principal`.

## 10. Example: `examples/auth`

Deliberately uses **`auth` alone — no autocrud** — to prove the crate stands on its own. A playground
that grows with the implementation. **Now:** a public page + a page to be gated (stub, ungated).
**Next (with `auth`):** a login page, a session cookie, and the "secret" page gated by `ValidUsers`;
then self-service password change. Gating an *autocrud* API with the same `auth` is shown by adding a
gate to the `autocrud`/`adminpanel` examples.

## 11. Decisions (confirmed)

1. **Packaging** — ✅ a **standalone `auth` crate** (authn + authz together; authz doesn't warrant its
   own crate). `autocrud → auth` via an optional feature; `auth` never depends on `autocrud`.
